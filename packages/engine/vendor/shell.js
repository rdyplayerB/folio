...
(function () {
  ...
  function autoHotspots(state) {
    ...
    for (var i = 0; i < list.length && i < AUTO_ROWS; i++) {
      out.push({ x: 6, y: AUTO_TOP + i * AUTO_STEP, w: 132, h: AUTO_H, obj: list[i] });
    }
    return out;
  }
  ...
  function drawFallbackScene() {
    ...
    for (var j = 0; j < list.length && j < AUTO_ROWS; j++) {
      ...
      frame(6, y, 132, AUTO_H, COL.dark);
      var drawn = false;
      if (GUE.sprites && typeof GUE.sprites.draw === 'function') {
        try { GUE.sprites.draw(ctx, list[j], 8, y - 2, t); drawn = true; } catch (e) { drawn = false; }
      }
      if (!drawn) { ctx.fillStyle = COL.dark; ctx.fillRect(9, y + 3, 5, 5); }
      text(labelOf(list[j]).slice(0, 14), 20, y + 2, COL.dim);
    }
    if (list.length > AUTO_ROWS) {
      text('+' + (list.length - AUTO_ROWS) + ' MORE', 20, AUTO_TOP + AUTO_ROWS * AUTO_STEP + 2, COL.dark);
    }
  }
  ...
  function drawInv() {
    ...
    for (var i = 0; i < slice.length; i++) {
      var id = slice[i];
      var x = INV.x + 5 + i * 20, y = (INV.y + (INV.h - 16) / 2) | 0;
      var drawn = false;
      if (GUE.sprites && typeof GUE.sprites.draw === 'function') {
        try { GUE.sprites.draw(ctx, id, x, y, t); drawn = true; } catch (e) { drawn = false; }
      }
      if (!drawn) {                                        // sprite missing: initial in a box
        frame(x, y, 16, 16, COL.dark);
        textC(String(id).charAt(0), x + 8, y + 4, COL.dim);
      }
      if (id === selectedItem) frame(x - 1, y - 1, 18, 18, COL.gold);
      invRects.push({ x: x - 1, y: y - 1, w: 18, h: 18, obj: id });
    }
  ...
  function onMove(e) {
    ...
    } else if (overClickable(p.x, p.y)) cur = 'pointer';
