...
| `exits` | **A map keyed by direction**, not a list — direction is the natural key and duplicates are meaningless. Live, not authored: computed from the logic layer's current truth, so a passage that opens mid-game appears without the presentation package being told. A backend that cannot compute live exits returns `{}` rather than throwing, and the view falls back to the game's static exit table. |
